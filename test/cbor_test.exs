defmodule CborTest do
  ...
  test "decode half precision float" do
    ...
    assert Cbor.decode(<<0xf9, 0x7c, 0x00>>) == {:ok, :infinity, <<>>}
    assert Cbor.decode(<<0xf9, 0x7e, 0x00>>) == {:ok, :nan, <<>>}
    assert Cbor.decode(<<0xf9, 0xfc, 0x00>>) == {:ok, :infinity, <<>>}
  end
  ...
  test "decode single precision float" do
    ...
    assert Cbor.decode(<<0xfa, 0x7f, 0x80, 0x00, 0x00>>) == {:ok, :infinity, <<>>}
    assert Cbor.decode(<<0xfa, 0x7f, 0xc0, 0x00, 0x00>>) == {:ok, :nan, <<>>}
    assert Cbor.decode(<<0xfa, 0xff, 0x80, 0x00, 0x00>>) == {:ok, :infinity, <<>>}
  end
  ...
  test "decode double precision float" do
    ...
    assert Cbor.decode(<<0xfb, 0x7f, 0xf0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00>>) == {:ok, :infinity, <<>>}
  ...
  test "encode byte string" do
    assert Cbor.encode(<<>>) == <<0x40>>
    assert Cbor.encode(<<0x01, 0x02, 0x03, 0x04>>) == <<0x44, 0x01, 0x02, 0x03, 0x04>>
  end

  test "encode text string" do
    assert Cbor.encode("a", :text_string) == <<0x61, 0x61>>
    assert Cbor.encode("IETF", :text_string) == <<0x64, 0x49, 0x45, 0x54, 0x46>>
    assert Cbor.encode("\"\\", :text_string) == <<0x62, 0x22, 0x5c>>
    assert Cbor.encode("\u00fc", :text_string) == <<0x62, 0xc3, 0xbc>>
    assert Cbor.encode("\u6c34", :text_string) == <<0x63, 0xe6, 0xb0, 0xb4>>
    assert Cbor.encode("a", :text_string) == <<0x61, 0x61>>
  end
end
